...
export default {
  ...
  data() {
    return {
      ...
      chart2: {
        listLoading: true,
        title: '班组-质量知识', // 会响应式刷新的不只是data里面的数据，还有vuex，vuex里面的数据一旦变化  使用该数据的地方也会刷新
        chartdata: [[], [], [], [], [], [], [], [], [], [], [], []],
        lengenddata: []
      }
    }
  ...
  watch: {
    year(newval) {
      ...
      getstationzhiliangzhishi(newval, this.chart1.chartdata, this.chart1.lengenddata).then(() => this.chart1.listLoading = false)
      getgroupzhiliangzhishi(newval, this.chart2.chartdata, this.chart2.lengenddata).then(() => this.chart2.listLoading = false)
    }
  }
}
</script>
