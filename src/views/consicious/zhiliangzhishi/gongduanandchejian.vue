...
import { getworkshopzhiliangzhishi, getzonezhiliangzhishi } from '@/api/qe/consicious'
export default {
  components: {
    Stackchart,
    SelectYear
  ...
  data() {
    return {
      chart1: {
        listLoading: true,
        title: '工段-质量知识', // 会响应式刷新的不只是data里面的数据，还有vuex，vuex里面的数据一旦变化  使用该数据的地方也会刷新
        chartdata: [[], [], [], [], [], [], [], [], [], [], [], []], // 每个数组里面都是一个月的从冲压 到装配的6个数据
        lengenddata: []
      },
      chart2: {
        listLoading: true,
        title: '车间-质量知识', // 会响应式刷新的不只是data里面的数据，还有vuex，vuex里面的数据一旦变化  使用该数据的地方也会刷新
        chartdata: [[], [], [], [], [], [], [], [], [], [], [], []],
        lengenddata: []
      }
    }
  ...
  mounted() {
    ...
    getzonezhiliangzhishi(this.year, this.chart2.chartdata, this.chart2.lengenddata).then(() => this.chart2.listLoading = false)
  },
  computed: {
    ...mapGetters(['year'])
  ...
  watch: {
    year(newval) {
      ...
      getworkshopzhiliangzhishi(newval, this.chart1.chartdata, this.chart1.lengenddata).then(() => this.chart1.listLoading = false)
      getzonezhiliangzhishi(newval, this.chart2.chartdata, this.chart2.lengenddata).then(() => this.chart2.listLoading = false)
    }
  }
}
</script>
