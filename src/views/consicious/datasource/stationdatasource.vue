...
export default { // 其实也就是个对象罢了
  ...
  methods: { // 一个拥有很多方法的对象
    getfilelist() {
      findallstation(this.listQuery.page, this.listQuery.limit, this.listQuery.sort).then(res => {
        console.log(res)
    ...
    handleDelete(row, index, id) { // 点击删除按钮的操作
      ...
      deletestationbyid(id).then(this.tableData.splice(index, 1))
    ...
    upload() {
      ...
      uploadstation(formData).then(res => { // upload1来自于import的方法
        ...
        if (res == 1) {
          ...
          that.$notify({ // 封装的通知功能
            title: '新增失败',
            message: '新增失败,模板第二行应当只有7列，请检查模板',
            type: 'error',
            duration: 30000
          })
        }
