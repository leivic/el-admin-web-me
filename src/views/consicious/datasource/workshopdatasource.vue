<template>
  <div>
    <!--toolbox-->
    <el-button v-permission="['consiciousworkshopadd']" size="large" type="nomal" @click="addfile">新增</el-button><!--enadd是监听子组件模板中的新增按钮，方法是本文件中定义的方法-->
    <!--工具栏-->
    ...
    >
      ...
      <el-table-column label="操作" width="100px" align="center">
        <template slot-scope="{row,$index}"><!--最开始的写法是 slot-scope="{row,$index}" 这个$index是vue2.0的key，在vue2.0的时候移除了-->
          <el-button v-permission="['consiciousworkshopdelete']" size="mini" type="nolmal" @click="handleDelete(row,index,row.id)">
            删除
          </el-button>
        </template>
      </el-table-column>
...
import { uploadworkshop, findallworkshop, deleteworkshopbyid } from '@/api/qe/consicious'
...
export default { // 其实也就是个对象罢了
  ...
  methods: { // 一个拥有很多方法的对象
    getfilelist() {
      findallworkshop(this.listQuery.page, this.listQuery.limit, this.listQuery.sort).then(res => {
        console.log(res)
    ...
    handleDelete(row, index, id) { // 点击删除按钮的操作
      ...
      deleteworkshopbyid(id).then(this.tableData.splice(index, 1))
