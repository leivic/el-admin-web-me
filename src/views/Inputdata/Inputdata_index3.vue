<template>
  <div class="Sidebar">
    <el-table :data="datalist" max-height="500px">
...
<script>
import { findAllIndexdata3, updateIndexdata3 } from '@/api/qe/other'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    submit(index, row) {
	    console.log(row)
	    updateIndexdata3(row.id, this.show1[index], this.show2[index], this.show3[index], this.show4[index], this.show5[index], this.show6[index]).then(res => {
		    this.$set(this.showEdit, index, false)
		    this.getList()
	    })
    }
